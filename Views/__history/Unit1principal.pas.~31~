...
type
  TSysform_principal = class(TForm)
    ...
    Image_rlmovimentaçao: TImage;
    Text_rlmovimentaçao: TText;
    ...
    cad_funçao: TMenuItem;
    cad_caixa: TMenuItem;
    cad_conta: TMenuItem;
    cad_serviço: TMenuItem;
    ...
    procedure venda_mesasClick(Sender: TObject);
...
begin
  // Implementação para outro manipulador de evento
end;
...
procedure TSysform_principal.venda_mesasClick(Sender: TObject);
begin
    // Verifica se o formulário já foi criado
  if not Assigned(Form_vdmesas) then
